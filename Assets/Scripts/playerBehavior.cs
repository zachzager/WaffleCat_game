...
public class playerBehavior : MonoBehaviour {
	...
	void Move () {
		...
		if (direction == RIGHT) { // right
			body.AddForce(transform.right * 2);
		} 
		...
		// Player 2 jump
		if (gameObject.tag == "Player 2" && Input.GetKeyDown  (KeyCode.UpArrow)) {
			body.AddForce(new Vector2(0,6), ForceMode2D.Impulse);
		}
		...
		if (Input.touchCount > 0) {
			...
			if (touch.phase == TouchPhase.Ended || touch.phase == TouchPhase.Canceled) {
				if (Time.time - TouchTime <= 0.5 && touch.position.x < Screen.width/2 && gameObject.tag == "Player 1") {
					body.AddForce(new Vector2(0,6), ForceMode2D.Impulse);
				}
				if (Time.time - TouchTime <= 0.5 && touch.position.x > Screen.width/2 && gameObject.tag == "Player 2") {
					body.AddForce(new Vector2(0,6), ForceMode2D.Impulse);
				}
			}
		}
